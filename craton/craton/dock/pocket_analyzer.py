...
class PocketAnalyzer:
    def __init__(self, pdb_file,csv_file=None,cavity_file=None,no_dispaly=True,step=0.6, probe_in=1.4, probe_out=4.0,
                 removal_distance=2.4, volume_cutoff=5.0, include_depth=True,
                 include_hydropathy=False, verbose=False
                 ):
        ...
        self.include_hydropathy = include_hydropathy
        self.verbose = verbose
        self.csv_file = csv_file
        self.cavity_file = cavity_file
        self.no_dispaly = no_dispaly

        self.results = None
        self.pockets = []


    def clean_LR(self, input_file, output_file):
        import re
        ...
        for line in lines:
            if line.startswith('ATOM') or line.startswith('HETATM'):
                # 提取残基编号部分(位置23-26, 0-indexed: 22-26)
                resnum_field = line[22:26].strip()
                
                # 移除字母,只保留数字
                resnum = re.sub(r'[^0-9]', '', resnum_field)
                ...
                # 重建PDB行,保持格式
                fixed_line = line[:22] + resnum.rjust(4) + line[26:]
    ...
    def detect_pockets(self):
        ...
        clean_LR_file = self.pdb_file + "_clean_LR.pdb" 
        self.clean_LR(self.pdb_file, clean_LR_file)    
        ...
        self.results = pyKVFinder.run_workflow(
                ...
                removal_distance=self.removal_distance,
                volume_cutoff=self.volume_cutoff,
                include_depth=self.include_depth,
                include_hydropathy=self.include_hydropathy,
    ...
    def export_cavity_pdb(self):
        if self.results is None:
            print("No results to export")
            return
        
        pdb_string = self.results.export(output=self.cavity_file)
        print(f"Cavity PDB exported to: {self.cavity_file}")
    ...
    def run(self):
        if self.detect_pockets():
            ...
            if self.cavity_file:
                self.export_cavity_pdb()
        
            return self.get_pockets_summary()
        else:
            return None
...
def main():
    ...
    parser.add_argument("-o", "--output", default=None, help="Export results to CSV file")
    ...
    parser.add_argument("--no-depth", default=False, action="store_true", 
    ...
    analyzer = PocketAnalyzer(args.pdb_file,csv_file=args.output,cavity_file=args.cavity_pdb,no_dispaly=args.no_display,
                        step=args.step, probe_in=args.probe_in, probe_out=args.probe_out, removal_distance=args.removal_distance,
                        volume_cutoff=args.volume_cutoff, include_depth=not args.no_depth,include_hydropathy=args.hydropathy,verbose=args.verbose)
